...
fn main() {
    ...
    println!(
        "case,events,telex_bytes,film_bytes,ratio,encode_ms,decode_ms,film_to_telex_ms,address_bytes,adjacent_address_prefix_bytes,datatype_bytes,repeated_datatype_bytes,value_bytes,repeated_value_bytes"
    );
    for case in cases {
        ...
        let film = encode_film_candidate_a(&stream, &[]).expect("generated Film must encode");
        let encode = measure(case.iterations, case.warmups, || {
            black_box(
                encode_film_candidate_a(black_box(&stream), &[])
                    .expect("generated Film must encode"),
            );
        });
        let decode = measure(case.iterations, case.warmups, || {
            black_box(
                decode_film_candidate_a(black_box(&film), &[]).expect("generated Film must decode"),
            );
        });
        let film_to_telex = measure(case.iterations, case.warmups, || {
            let decoded =
                decode_film_candidate_a(black_box(&film), &[]).expect("generated Film must decode");
            black_box(
        ...
        println!(
            ...
            telex.len(),
            film.len(),
            film.len() as f64 / telex.len() as f64,
            milliseconds(encode),
            milliseconds(decode),
            milliseconds(film_to_telex),
